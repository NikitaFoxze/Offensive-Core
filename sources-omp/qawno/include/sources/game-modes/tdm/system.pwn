...
static
	actor_top_kills[TDM_MAX_GAME_SESSIONS][5],
	Text3D:actor_3DText[TDM_MAX_GAME_SESSIONS][5],

	actor_Data[TDM_MAX_GAME_SESSIONS][MAX_PLAYERS][3],
	actor_Name[TDM_MAX_GAME_SESSIONS][MAX_PLAYERS][MAX_PLAYER_NAME],
	actor_Team[TDM_MAX_GAME_SESSIONS][MAX_PLAYERS],
	actor_Skin[TDM_MAX_GAME_SESSIONS][MAX_PLAYERS],
	actor_tempVar[TDM_MAX_GAME_SESSIONS];
...
stock TDM_GivePlayerWeapon(playerid, weaponid, ammo)
{
	...
	GetPlayerWeaponData(playerid, GetWeaponSlotEx(weaponid), d_weaponid, d_ammo);
...
stock TDM_HideSelectedPlayerTP(playerid, bool:iterremove = true)
{
	...
	&& ActiveCellSelectPlayerID[playerid] != -2) {
		...
		PlayerTextDrawBackgroundColour(playerid, TD_SelectPlayer[playerid][ActiveCellSelectPlayerTD[playerid]], -1717986817);
...
stock TDM_HideSelectedPointTP(playerid)
{
	if(ActiveCellSelectPoint[playerid] != -1) {
		PlayerTextDrawBackgroundColour(playerid, TD_SelectPoint[playerid][ActiveCellSelectPointTD[playerid]], -1717986817);
		...
		if(Interface_IsOpen(playerid, Interface:TDM_SelectTP)) {
			if(UpdateTimerSelectTP[playerid] < gettime()) { 
				...
				foreach(new g:TDM_CapturePointCount[session_id]) {
					// Квадрат точки
					PlayerTextDrawBackgroundColour(playerid, TD_SelectPoint[playerid][tt], TDM_ShowTeamColorXF(TDM_GetPointTeam(session_id, g)));
					...
					PlayerTextDrawColour(playerid, TD_SelectPoint[playerid][tt], TDM_ShowTeamColorXF(TDM_GetPointTeam(session_id, g)));
					PlayerTextDrawShow(playerid, TD_SelectPoint[playerid][tt]);

					// Название точки
					PlayerTextDrawColour(playerid, TD_SelectPoint[playerid][tt + 1], TDM_ShowTeamColorXF(TDM_GetPointTeam(session_id, g)));
...
static LocationEndGame(session_id, next_location = -1) 
{
	...
	m_for(MODE_TDM, session_id, p) {
		...
		actor_Data[session_id][actor_tempVar[session_id]][0] = Mode_GetPlayerKills(p);
		actor_Data[session_id][actor_tempVar[session_id]][2] = Mode_GetPlayerDeaths(p);
		GetPlayerName(p, actor_Name[session_id][p], MAX_PLAYER_NAME);
		actor_Data[session_id][actor_tempVar[session_id]++][1] = p;

		actor_Team[session_id][p] = GetPlayerTeamEx(p);

		if(GetPlayerSex(p) == SEX_MALE)
			actor_Skin[session_id][p] = TDM_GetClassSkin(GetPlayerTeamEx(p), GetPlayerCustomClass(p), 0);

		else if(GetPlayerSex(p) == SEX_FEMALE)
			actor_Skin[session_id][p] = TDM_GetClassSkin(GetPlayerTeamEx(p), GetPlayerCustomClass(p), 1);
	}
	for(new i = 0, j = 0; i < actor_tempVar[session_id]; i++) {
		j = actor_Data[session_id][i][0];

		for(new k = i - 1; k > -1; k--) {
			if(j > actor_Data[session_id][k][0]) {
				actor_Data[session_id][k][0] ^= actor_Data[session_id][k + 1][0], actor_Data[session_id][k + 1][0] ^= actor_Data[session_id][k][0], actor_Data[session_id][k][0] ^= actor_Data[session_id][k + 1][0];
				actor_Data[session_id][k][1] ^= actor_Data[session_id][k + 1][1], actor_Data[session_id][k + 1][1] ^= actor_Data[session_id][k][1], actor_Data[session_id][k][1] ^= actor_Data[session_id][k + 1][1];
			}
		}
	}

	// Актёры топ 5 убийств
	n_for(actorid, 5) {
		if(actor_Data[session_id][actorid][1] > -1) {
			new
				Float:X, Float:Y, Float:Z, Float:Angle,
				actor_id = actor_Data[session_id][actorid][1];

			TDM_GetSpawnTopActor(session_id, actorid, X, Y, Z, Angle);

			actor_top_kills[session_id][actorid] = CreateDynamicActor(actor_Skin[session_id][actor_id], X, Y, Z, Angle, 1, 100.0, Mode_GetInterior(MODE_TDM, session_id), Mode_GetVirtualWorld(MODE_TDM, session_id));

			switch(actorid) {
				case 0: ApplyDynamicActorAnimation(actor_top_kills[session_id][actorid], "BENCHPRESS", "GYM_BP_CELEBRATE", 4.0, 1, 0, 0, 0, 0);
				case 1: ApplyDynamicActorAnimation(actor_top_kills[session_id][actorid], "PED", "XPRESSSCRATCH", 4.0, 1, 0, 0, 0, 0);
				case 2: ApplyDynamicActorAnimation(actor_top_kills[session_id][actorid], "PED", "IDLE_HBHB", 4.0, 1, 0, 0, 0, 0);
				case 3: ApplyDynamicActorAnimation(actor_top_kills[session_id][actorid], "PED", "IDLE_TIRED", 4.0, 1, 0, 0, 0, 0);
				case 4: ApplyDynamicActorAnimation(actor_top_kills[session_id][actorid], "PED", "SHOT_PARTIAL", 4.0, 1, 0, 0, 0, 0);
			}

			new
				str[100];

			f(str, "{FFFFFF}%i Место\n%s%s\n{FFFFFF}Убийств: {d91616}%i", actorid + 1, TDM_ShowTeamColor(actor_Team[session_id][actor_id]), actor_Name[session_id][actor_id], actor_Data[session_id][actorid][0]);
			actor_3DText[session_id][actorid] = CreateDynamic3DTextLabel(str, -1, 0.0, 0.0, -1.0, 50.0, actor_top_kills[session_id][actorid], INVALID_VEHICLE_ID, Mode_GetVirtualWorld(MODE_TDM, session_id), Mode_GetInterior(MODE_TDM, session_id));
		}
	}
...
{
	n_for(actorid, 5) {
		if(IsValidDynamicActor(actor_top_kills[session_id][actorid]))
			DestroyDynamicActor(actor_top_kills[session_id][actorid]);

		if(IsValidDynamic3DTextLabel(actor_3DText[session_id][actorid]))
			DestroyDynamic3DTextLabel(actor_3DText[session_id][actorid]);
	}

	ResetTopPlayers(session_id);
	ResetTopActors(session_id);
	return 1;
}
...
static ShowTDTimerNextLocation(playerid)
{
	...
	PlayerTextDrawAlignment(playerid, TD_TNextLocation[playerid], TEXT_DRAW_ALIGN_CENTER);
	PlayerTextDrawColour(playerid, TD_TNextLocation[playerid], 0xFFFFFFFF);
	PlayerTextDrawUseBox(playerid, TD_TNextLocation[playerid], true);
	PlayerTextDrawBoxColour(playerid, TD_TNextLocation[playerid], 1768516095);
	PlayerTextDrawBackgroundColour(playerid, TD_TNextLocation[playerid], 1768515993);
	PlayerTextDrawFont(playerid, TD_TNextLocation[playerid], TEXT_DRAW_FONT_BANK_GOTHIC);
	PlayerTextDrawSetProportional(playerid, TD_TNextLocation[playerid], true);
...
static ShowPlayerTDTextTeamMatch(playerid)
{
	...
	foreach(new tt:TDM_ActiveTeams[session_id]) {
		...
		PlayerTextDrawAlignment(playerid, TD_TextTeamMatch[playerid][tt], TEXT_DRAW_ALIGN_CENTER);
		PlayerTextDrawColour(playerid, TD_TextTeamMatch[playerid][tt], -858993409);
		...
		PlayerTextDrawBackgroundColour(playerid, TD_TextTeamMatch[playerid][tt], 48);
		PlayerTextDrawFont(playerid, TD_TextTeamMatch[playerid][tt], TEXT_DRAW_FONT_BANK_GOTHIC);
		PlayerTextDrawSetProportional(playerid, TD_TextTeamMatch[playerid][tt], true);
		PlayerTextDrawSetShadow(playerid, TD_TextTeamMatch[playerid][tt], 0);
	}
	return 1;
}
...
static ResetTopActors(session_id)
{
	n_for(b, 5) {
		actor_3DText[session_id][b] = INVALID_DYNAMIC_3D_TEXT_ID;

		actor_Data[session_id][b][0] =
		actor_Data[session_id][b][1] =
		actor_Data[session_id][b][2] = -1;
		actor_Name[session_id][b][0] = EOS;
		actor_Team[session_id][b] =
		actor_Skin[session_id][b] = 0;
	}
	actor_tempVar[session_id] = 0;
	return 1;
}
...
static CreatePlayerTDTeamChet(playerid, td_id = -1, one_td_id = -1, team_id, maxchet)
{
	...
	if(one_td_id != -1) {
		switch(one_td_id) {
			case 0: {
				...
				PlayerTextDrawAlignment(playerid, TD_ChetLocation[playerid][0], TEXT_DRAW_ALIGN_CENTER);
				PlayerTextDrawColour(playerid, TD_ChetLocation[playerid][0], TDM_ShowTeamColorXB(team_id));
				...
				PlayerTextDrawBackgroundColour(playerid, TD_ChetLocation[playerid][0], 255);
				PlayerTextDrawFont(playerid, TD_ChetLocation[playerid][0], TEXT_DRAW_FONT_BANK_GOTHIC);
				PlayerTextDrawSetProportional(playerid, TD_ChetLocation[playerid][0], true);
				...
				PlayerTextDrawAlignment(playerid, TD_ChetLocation[playerid][1], TEXT_DRAW_ALIGN_CENTER);
				PlayerTextDrawColour(playerid, TD_ChetLocation[playerid][1], 0xFFFFFFFF);
				PlayerTextDrawSetOutline(playerid, TD_ChetLocation[playerid][1], 0);
				PlayerTextDrawBackgroundColour(playerid, TD_ChetLocation[playerid][1], 255);
				PlayerTextDrawFont(playerid, TD_ChetLocation[playerid][1], TEXT_DRAW_FONT_BANK_GOTHIC);
				PlayerTextDrawSetProportional(playerid, TD_ChetLocation[playerid][1], true);
			...
			case 2: {
				...
				PlayerTextDrawAlignment(playerid, TD_ChetLocation[playerid][4], TEXT_DRAW_ALIGN_CENTER);
				PlayerTextDrawColour(playerid, TD_ChetLocation[playerid][4], TDM_ShowTeamColorXB(team_id));
				...
				PlayerTextDrawBackgroundColour(playerid, TD_ChetLocation[playerid][4], 255);
				PlayerTextDrawFont(playerid, TD_ChetLocation[playerid][4], TEXT_DRAW_FONT_BANK_GOTHIC);
				PlayerTextDrawSetProportional(playerid, TD_ChetLocation[playerid][4], true);
				...
				PlayerTextDrawAlignment(playerid, TD_ChetLocation[playerid][5], TEXT_DRAW_ALIGN_CENTER);
				PlayerTextDrawColour(playerid, TD_ChetLocation[playerid][5], 0xFFFFFFFF);
				PlayerTextDrawSetOutline(playerid, TD_ChetLocation[playerid][5], 0);
				PlayerTextDrawBackgroundColour(playerid, TD_ChetLocation[playerid][5], 255);
				PlayerTextDrawFont(playerid, TD_ChetLocation[playerid][5], TEXT_DRAW_FONT_BANK_GOTHIC);
				PlayerTextDrawSetProportional(playerid, TD_ChetLocation[playerid][5], true);
	...
	if(td_id != -1) {
		switch(td_id) {
			case 0: {
				PlayerChetLocation[playerid][0] = CreatePlayerProgressBar(playerid, 233.00, 43.00, 84.50, 5.19, TDM_ShowTeamColorXB(team_id), maxchet, BAR_DIRECTION_LEFT);

				// Название команды
				TD_ChetLocation[playerid][0] = CreatePlayerTextDraw(playerid, 316.000000, 29.000000, str);
				PlayerTextDrawLetterSize(playerid, TD_ChetLocation[playerid][0], 0.322999, 1.259852);
				PlayerTextDrawAlignment(playerid, TD_ChetLocation[playerid][0], TEXT_DRAW_ALIGN_RIGHT);
				PlayerTextDrawColour(playerid, TD_ChetLocation[playerid][0], TDM_ShowTeamColorXB(team_id));
				PlayerTextDrawSetShadow(playerid, TD_ChetLocation[playerid][0], 1);
				PlayerTextDrawSetOutline(playerid, TD_ChetLocation[playerid][0], 0);
				PlayerTextDrawBackgroundColour(playerid, TD_ChetLocation[playerid][0], 255);
				...
				PlayerTextDrawLetterSize(playerid, TD_ChetLocation[playerid][1], 0.296999, 1.081482);
				PlayerTextDrawAlignment(playerid, TD_ChetLocation[playerid][1], TEXT_DRAW_ALIGN_RIGHT);
				PlayerTextDrawColour(playerid, TD_ChetLocation[playerid][1], 0xFFFFFFFF);
				PlayerTextDrawSetOutline(playerid, TD_ChetLocation[playerid][1], 0);
				PlayerTextDrawBackgroundColour(playerid, TD_ChetLocation[playerid][1], 255);
				PlayerTextDrawFont(playerid, TD_ChetLocation[playerid][1], TEXT_DRAW_FONT_BANK_GOTHIC);
			...
			case 1: {
				...
				PlayerTextDrawAlignment(playerid, TD_ChetLocation[playerid][2], TEXT_DRAW_ALIGN_LEFT);
				PlayerTextDrawColour(playerid, TD_ChetLocation[playerid][2], TDM_ShowTeamColorXB(team_id));
				...
				PlayerTextDrawBackgroundColour(playerid, TD_ChetLocation[playerid][2], 255);
				PlayerTextDrawFont(playerid, TD_ChetLocation[playerid][2], TEXT_DRAW_FONT_BANK_GOTHIC);
				PlayerTextDrawSetProportional(playerid, TD_ChetLocation[playerid][2], true);
				...
				PlayerTextDrawAlignment(playerid, TD_ChetLocation[playerid][3], TEXT_DRAW_ALIGN_LEFT);
				PlayerTextDrawColour(playerid, TD_ChetLocation[playerid][3], 0xFFFFFFFF);
				PlayerTextDrawSetOutline(playerid, TD_ChetLocation[playerid][3], 0);
				PlayerTextDrawBackgroundColour(playerid, TD_ChetLocation[playerid][3], 255);
				PlayerTextDrawFont(playerid, TD_ChetLocation[playerid][3], TEXT_DRAW_FONT_BANK_GOTHIC);
				PlayerTextDrawSetProportional(playerid, TD_ChetLocation[playerid][3], true);
			...
			case 2: {
				PlayerChetLocation[playerid][2] = CreatePlayerProgressBar(playerid, 233.00, 65.00, 84.50, 5.19, TDM_ShowTeamColorXB(team_id), maxchet, BAR_DIRECTION_LEFT);

				TD_ChetLocation[playerid][4] = CreatePlayerTextDraw(playerid, 316.000000, 51.000000, str);
				PlayerTextDrawLetterSize(playerid, TD_ChetLocation[playerid][4], 0.322999, 1.259852);
				PlayerTextDrawAlignment(playerid, TD_ChetLocation[playerid][4], TEXT_DRAW_ALIGN_RIGHT);
				PlayerTextDrawColour(playerid, TD_ChetLocation[playerid][4], TDM_ShowTeamColorXB(team_id));
				PlayerTextDrawSetShadow(playerid, TD_ChetLocation[playerid][4], 1);
				PlayerTextDrawSetOutline(playerid, TD_ChetLocation[playerid][4], 0);
				PlayerTextDrawBackgroundColour(playerid, TD_ChetLocation[playerid][4], 255);
				...
				PlayerTextDrawLetterSize(playerid, TD_ChetLocation[playerid][5], 0.296999, 1.081482);
				PlayerTextDrawAlignment(playerid, TD_ChetLocation[playerid][5], TEXT_DRAW_ALIGN_RIGHT);
				PlayerTextDrawColour(playerid, TD_ChetLocation[playerid][5], 0xFFFFFFFF);
				PlayerTextDrawSetOutline(playerid, TD_ChetLocation[playerid][5], 0);
				PlayerTextDrawBackgroundColour(playerid, TD_ChetLocation[playerid][5], 255);
				PlayerTextDrawFont(playerid, TD_ChetLocation[playerid][5], TEXT_DRAW_FONT_BANK_GOTHIC);
			...
			case 3: {
				...
				PlayerTextDrawAlignment(playerid, TD_ChetLocation[playerid][6], TEXT_DRAW_ALIGN_LEFT);
				PlayerTextDrawColour(playerid, TD_ChetLocation[playerid][6], TDM_ShowTeamColorXB(team_id));
				...
				PlayerTextDrawBackgroundColour(playerid, TD_ChetLocation[playerid][6], 255);
				PlayerTextDrawFont(playerid, TD_ChetLocation[playerid][6], TEXT_DRAW_FONT_BANK_GOTHIC);
				PlayerTextDrawSetProportional(playerid, TD_ChetLocation[playerid][6], true);
				...
				PlayerTextDrawAlignment(playerid, TD_ChetLocation[playerid][7], TEXT_DRAW_ALIGN_LEFT);
				PlayerTextDrawColour(playerid, TD_ChetLocation[playerid][7], 0xFFFFFFFF);
				PlayerTextDrawSetOutline(playerid, TD_ChetLocation[playerid][7], 0);
				PlayerTextDrawBackgroundColour(playerid, TD_ChetLocation[playerid][7], 255);
				PlayerTextDrawFont(playerid, TD_ChetLocation[playerid][7], TEXT_DRAW_FONT_BANK_GOTHIC);
				PlayerTextDrawSetProportional(playerid, TD_ChetLocation[playerid][7], true);
...
{
	// Белый фон начать
	TD_SelectSpawn[playerid][0] = CreatePlayerTextDraw(playerid, 168.0000, 417.0000, "_");
	PlayerTextDrawFont(playerid, TD_SelectSpawn[playerid][0], TEXT_DRAW_FONT_PREVIEW);
	PlayerTextDrawAlignment(playerid, TD_SelectSpawn[playerid][0], TEXT_DRAW_ALIGN_LEFT);
	PlayerTextDrawColour(playerid, TD_SelectSpawn[playerid][0], 0xFFFFFFFF);
	PlayerTextDrawBackgroundColour(playerid, TD_SelectSpawn[playerid][0], -1717986817);
	PlayerTextDrawSetProportional(playerid, TD_SelectSpawn[playerid][0], false);
	...
	TD_SelectSpawn[playerid][1] = CreatePlayerTextDraw(playerid, 169.0000, 418.0000, "_");
	PlayerTextDrawFont(playerid, TD_SelectSpawn[playerid][1], TEXT_DRAW_FONT_PREVIEW);
	PlayerTextDrawAlignment(playerid, TD_SelectSpawn[playerid][1], TEXT_DRAW_ALIGN_LEFT);
	PlayerTextDrawColour(playerid, TD_SelectSpawn[playerid][1], 0xFFFFFFFF);
	PlayerTextDrawBackgroundColour(playerid, TD_SelectSpawn[playerid][1], 303174399);
	PlayerTextDrawSetProportional(playerid, TD_SelectSpawn[playerid][1], false);
	...
	PlayerTextDrawAlignment(playerid, TD_SelectSpawn[playerid][2], TEXT_DRAW_ALIGN_CENTER);
	PlayerTextDrawColour(playerid, TD_SelectSpawn[playerid][2], -949919233);
	PlayerTextDrawBackgroundColour(playerid, TD_SelectSpawn[playerid][2], 255);
	PlayerTextDrawFont(playerid, TD_SelectSpawn[playerid][2], TEXT_DRAW_FONT_BANK_GOTHIC);
	PlayerTextDrawSetProportional(playerid, TD_SelectSpawn[playerid][2], true);
	...
	TD_SelectSpawn[playerid][3] = CreatePlayerTextDraw(playerid, 218.0000, 417.0000, "_");
	PlayerTextDrawFont(playerid, TD_SelectSpawn[playerid][3], TEXT_DRAW_FONT_PREVIEW);
	PlayerTextDrawAlignment(playerid, TD_SelectSpawn[playerid][3], TEXT_DRAW_ALIGN_LEFT);
	PlayerTextDrawColour(playerid, TD_SelectSpawn[playerid][3], 0xFFFFFFFF);
	PlayerTextDrawBackgroundColour(playerid, TD_SelectSpawn[playerid][3], -1717986817);
	PlayerTextDrawSetProportional(playerid, TD_SelectSpawn[playerid][3], false);
	...
	TD_SelectSpawn[playerid][4] = CreatePlayerTextDraw(playerid, 219.0000, 418.0000, "_");
	PlayerTextDrawFont(playerid, TD_SelectSpawn[playerid][4], TEXT_DRAW_FONT_PREVIEW);
	PlayerTextDrawAlignment(playerid, TD_SelectSpawn[playerid][4], TEXT_DRAW_ALIGN_LEFT);
	PlayerTextDrawColour(playerid, TD_SelectSpawn[playerid][4], 0xFFFFFFFF);
	PlayerTextDrawBackgroundColour(playerid, TD_SelectSpawn[playerid][4], 303174399);
	PlayerTextDrawSetProportional(playerid, TD_SelectSpawn[playerid][4], false);
	...
	PlayerTextDrawAlignment(playerid, TD_SelectSpawn[playerid][5], TEXT_DRAW_ALIGN_CENTER);
	PlayerTextDrawColour(playerid, TD_SelectSpawn[playerid][5], 1636288511);
	PlayerTextDrawBackgroundColour(playerid, TD_SelectSpawn[playerid][5], 255);
	PlayerTextDrawFont(playerid, TD_SelectSpawn[playerid][5], TEXT_DRAW_FONT_BANK_GOTHIC);
	PlayerTextDrawSetProportional(playerid, TD_SelectSpawn[playerid][5], true);
	...
	// Белый фон квестов
	TD_SelectSpawn[playerid][6] = CreatePlayerTextDraw(playerid, 336.0000, 417.0000, "_");
	PlayerTextDrawFont(playerid, TD_SelectSpawn[playerid][6], TEXT_DRAW_FONT_PREVIEW);
	PlayerTextDrawAlignment(playerid, TD_SelectSpawn[playerid][6], TEXT_DRAW_ALIGN_LEFT);
	PlayerTextDrawColour(playerid, TD_SelectSpawn[playerid][6], 0xFFFFFFFF);
	PlayerTextDrawBackgroundColour(playerid, TD_SelectSpawn[playerid][6], -1717986817);
	PlayerTextDrawSetProportional(playerid, TD_SelectSpawn[playerid][6], false);
	...
	TD_SelectSpawn[playerid][7] = CreatePlayerTextDraw(playerid, 337.0000, 418.0000, "_");
	PlayerTextDrawFont(playerid, TD_SelectSpawn[playerid][7], TEXT_DRAW_FONT_PREVIEW);
	PlayerTextDrawAlignment(playerid, TD_SelectSpawn[playerid][7], TEXT_DRAW_ALIGN_LEFT);
	PlayerTextDrawColour(playerid, TD_SelectSpawn[playerid][7], 0xFFFFFFFF);
	PlayerTextDrawBackgroundColour(playerid, TD_SelectSpawn[playerid][7], 303174399);
	PlayerTextDrawSetProportional(playerid, TD_SelectSpawn[playerid][7], false);
	...
	PlayerTextDrawAlignment(playerid, TD_SelectSpawn[playerid][8], TEXT_DRAW_ALIGN_CENTER);
	PlayerTextDrawColour(playerid, TD_SelectSpawn[playerid][8], -910794241);
	PlayerTextDrawBackgroundColour(playerid, TD_SelectSpawn[playerid][8], 255);
	PlayerTextDrawFont(playerid, TD_SelectSpawn[playerid][8], TEXT_DRAW_FONT_BANK_GOTHIC);
	PlayerTextDrawSetProportional(playerid, TD_SelectSpawn[playerid][8], true);
	...
	TD_SelectSpawn[playerid][9] = CreatePlayerTextDraw(playerid, 386.0000, 417.0000, "_");
	PlayerTextDrawFont(playerid, TD_SelectSpawn[playerid][9], TEXT_DRAW_FONT_PREVIEW);
	PlayerTextDrawAlignment(playerid, TD_SelectSpawn[playerid][9], TEXT_DRAW_ALIGN_LEFT);
	PlayerTextDrawColour(playerid, TD_SelectSpawn[playerid][9], 0xFFFFFFFF);
	PlayerTextDrawBackgroundColour(playerid, TD_SelectSpawn[playerid][9], -1717986817);
	PlayerTextDrawSetProportional(playerid, TD_SelectSpawn[playerid][9], false);
	...
	TD_SelectSpawn[playerid][10] = CreatePlayerTextDraw(playerid, 387.0000, 418.0000, "_");
	PlayerTextDrawFont(playerid, TD_SelectSpawn[playerid][10], TEXT_DRAW_FONT_PREVIEW);
	PlayerTextDrawAlignment(playerid, TD_SelectSpawn[playerid][10], TEXT_DRAW_ALIGN_LEFT);
	PlayerTextDrawColour(playerid, TD_SelectSpawn[playerid][10], 0xFFFFFFFF);
	PlayerTextDrawBackgroundColour(playerid, TD_SelectSpawn[playerid][10], 303174399);
	PlayerTextDrawSetProportional(playerid, TD_SelectSpawn[playerid][10], false);
	...
	PlayerTextDrawAlignment(playerid, TD_SelectSpawn[playerid][11], TEXT_DRAW_ALIGN_CENTER);
	PlayerTextDrawColour(playerid, TD_SelectSpawn[playerid][11], -1429773825);
	PlayerTextDrawBackgroundColour(playerid, TD_SelectSpawn[playerid][11], 255);
	PlayerTextDrawFont(playerid, TD_SelectSpawn[playerid][11], TEXT_DRAW_FONT_BANK_GOTHIC);
	PlayerTextDrawSetProportional(playerid, TD_SelectSpawn[playerid][11], true);
...
static CreatePlayerTDSelectPoint(playerid) 
{
	...
	if(point > 0) {
		for(new i = 0, td = 0, points = point - 1; i < point; i++) {
			// Белый фон точки
			TD_SelectPoint[playerid][td] = CreatePlayerTextDraw(playerid, pos_td[points][i][0], pos_td[points][i][1], "_");
			PlayerTextDrawFont(playerid, TD_SelectPoint[playerid][td], TEXT_DRAW_FONT_PREVIEW);
			PlayerTextDrawAlignment(playerid, TD_SelectPoint[playerid][td], TEXT_DRAW_ALIGN_LEFT);
			PlayerTextDrawColour(playerid, TD_SelectPoint[playerid][td], 0xFFFFFFFF);
			PlayerTextDrawBackgroundColour(playerid, TD_SelectPoint[playerid][td], -1717986817);
			PlayerTextDrawSetProportional(playerid, TD_SelectPoint[playerid][td], false);
			...
			// Кликабельный фон
			TD_SelectPoint[playerid][td] = CreatePlayerTextDraw(playerid, pos_td[points][i][0] + 1.0, pos_td[points][i][1] + 1.0, "_");
			PlayerTextDrawFont(playerid, TD_SelectPoint[playerid][td], TEXT_DRAW_FONT_PREVIEW);
			PlayerTextDrawAlignment(playerid, TD_SelectPoint[playerid][td], TEXT_DRAW_ALIGN_LEFT);
			PlayerTextDrawColour(playerid, TD_SelectPoint[playerid][td], 0xFFFFFFFF);
			PlayerTextDrawBackgroundColour(playerid, TD_SelectPoint[playerid][td], 303174399);
			PlayerTextDrawSetProportional(playerid, TD_SelectPoint[playerid][td], false);
			PlayerTextDrawSetShadow(playerid, TD_SelectPoint[playerid][td], 0);
			...
			// Задний фон точки
			TD_SelectPoint[playerid][td] = CreatePlayerTextDraw(playerid, pos_td[points][i][0] + 14.0, pos_td[points][i][1] + 3.0, "_");
			PlayerTextDrawFont(playerid, TD_SelectPoint[playerid][td], TEXT_DRAW_FONT_PREVIEW);
			PlayerTextDrawAlignment(playerid, TD_SelectPoint[playerid][td], TEXT_DRAW_ALIGN_LEFT);
			PlayerTextDrawColour(playerid, TD_SelectPoint[playerid][td], 0xFFFFFFFF);
			PlayerTextDrawBackgroundColour(playerid, TD_SelectPoint[playerid][td], TDM_ShowTeamColorXF(TDM_GetPointTeam(session_id, i)));
			PlayerTextDrawSetProportional(playerid, TD_SelectPoint[playerid][td], false);
			PlayerTextDrawSetShadow(playerid, TD_SelectPoint[playerid][td], 0);
			...
			PlayerTextDrawAlignment(playerid, TD_SelectPoint[playerid][td], TEXT_DRAW_ALIGN_CENTER);
			PlayerTextDrawColour(playerid, TD_SelectPoint[playerid][td], -101058561);
			PlayerTextDrawSetOutline(playerid, TD_SelectPoint[playerid][td], 1);
			PlayerTextDrawBackgroundColour(playerid, TD_SelectPoint[playerid][td], 144);
			PlayerTextDrawFont(playerid, TD_SelectPoint[playerid][td], TEXT_DRAW_FONT_BANK_GOTHIC);
			PlayerTextDrawSetProportional(playerid, TD_SelectPoint[playerid][td], true);
			PlayerTextDrawSetShadow(playerid, TD_SelectPoint[playerid][td], 0);
			td++;
			...
			PlayerTextDrawAlignment(playerid, TD_SelectPoint[playerid][td], TEXT_DRAW_ALIGN_CENTER);
			PlayerTextDrawColour(playerid, TD_SelectPoint[playerid][td], TDM_ShowTeamColorXF(TDM_GetPointTeam(session_id, i)));
			PlayerTextDrawBackgroundColour(playerid, TD_SelectPoint[playerid][td], 80);
			PlayerTextDrawFont(playerid, TD_SelectPoint[playerid][td], TEXT_DRAW_FONT_AHARONI_BOLD);
			PlayerTextDrawSetProportional(playerid, TD_SelectPoint[playerid][td], true);
			PlayerTextDrawSetShadow(playerid, TD_SelectPoint[playerid][td], 0);
			td++;
			str[0] = EOS;
...
{
	// Основной задний фон
	TD_SelectNextLocation[playerid][0] = CreatePlayerTextDraw(playerid, 226.0000, 117.0000, "_");
	PlayerTextDrawFont(playerid, TD_SelectNextLocation[playerid][0], TEXT_DRAW_FONT_PREVIEW);
	PlayerTextDrawAlignment(playerid, TD_SelectNextLocation[playerid][0], TEXT_DRAW_ALIGN_LEFT);
	PlayerTextDrawColour(playerid, TD_SelectNextLocation[playerid][0], 0xFFFFFFFF);
	PlayerTextDrawBackgroundColour(playerid, TD_SelectNextLocation[playerid][0], 976894719);
	PlayerTextDrawSetProportional(playerid, TD_SelectNextLocation[playerid][0], false);
	...
	// Основной задний фон
	TD_SelectNextLocation[playerid][1] = CreatePlayerTextDraw(playerid, 226.0000, 117.0000, "_");
	PlayerTextDrawFont(playerid, TD_SelectNextLocation[playerid][1], TEXT_DRAW_FONT_PREVIEW);
	PlayerTextDrawAlignment(playerid, TD_SelectNextLocation[playerid][1], TEXT_DRAW_ALIGN_LEFT);
	PlayerTextDrawColour(playerid, TD_SelectNextLocation[playerid][1], 0xFFFFFFFF);
	PlayerTextDrawBackgroundColour(playerid, TD_SelectNextLocation[playerid][1], 757935615);
	PlayerTextDrawSetProportional(playerid, TD_SelectNextLocation[playerid][1], false);
	...
	PlayerTextDrawAlignment(playerid, TD_SelectNextLocation[playerid][2], TEXT_DRAW_ALIGN_CENTER);
	PlayerTextDrawColour(playerid, TD_SelectNextLocation[playerid][2], -505290753);
	PlayerTextDrawBackgroundColour(playerid, TD_SelectNextLocation[playerid][2], 255);
	PlayerTextDrawFont(playerid, TD_SelectNextLocation[playerid][2], TEXT_DRAW_FONT_BANK_GOTHIC);
	PlayerTextDrawSetProportional(playerid, TD_SelectNextLocation[playerid][2], true);
	...
	for(new td = 3, s = 1; td < sizeof(TD_SelectNextLocation[]); td++, s++) {
		// Чёрный фон
		TD_SelectNextLocation[playerid][td] = CreatePlayerTextDraw(playerid, initialcoords[0], initialcoords[1], "_");
		PlayerTextDrawFont(playerid, TD_SelectNextLocation[playerid][td], TEXT_DRAW_FONT_PREVIEW);
		PlayerTextDrawAlignment(playerid, TD_SelectNextLocation[playerid][td], TEXT_DRAW_ALIGN_LEFT);
		PlayerTextDrawColour(playerid, TD_SelectNextLocation[playerid][td], 0xFFFFFFFF);
		PlayerTextDrawBackgroundColour(playerid, TD_SelectNextLocation[playerid][td], 303174399);
		PlayerTextDrawSetProportional(playerid, TD_SelectNextLocation[playerid][td], false);
		...
		PlayerTextDrawAlignment(playerid, TD_SelectNextLocation[playerid][td], TEXT_DRAW_ALIGN_CENTER);
		PlayerTextDrawColour(playerid, TD_SelectNextLocation[playerid][td], -394258433);
		PlayerTextDrawBackgroundColour(playerid, TD_SelectNextLocation[playerid][td], -394258433);
		PlayerTextDrawFont(playerid, TD_SelectNextLocation[playerid][td], TEXT_DRAW_FONT_BANK_GOTHIC);
		PlayerTextDrawSetProportional(playerid, TD_SelectNextLocation[playerid][td], true);
		PlayerTextDrawSetShadow(playerid, TD_SelectNextLocation[playerid][td], 0);
		td++;
		...
		PlayerTextDrawTextSize(playerid, TD_SelectNextLocation[playerid][td], 0.0000, 51.0000);
		PlayerTextDrawAlignment(playerid, TD_SelectNextLocation[playerid][td], TEXT_DRAW_ALIGN_CENTER);
		PlayerTextDrawColour(playerid, TD_SelectNextLocation[playerid][td], -648201473);
		PlayerTextDrawBackgroundColour(playerid, TD_SelectNextLocation[playerid][td], -394258433);
		PlayerTextDrawFont(playerid, TD_SelectNextLocation[playerid][td], TEXT_DRAW_FONT_BANK_GOTHIC);
		PlayerTextDrawSetProportional(playerid, TD_SelectNextLocation[playerid][td], true);
		PlayerTextDrawSetShadow(playerid, TD_SelectNextLocation[playerid][td], 0);
		...
		PlayerTextDrawTextSize(playerid, TD_SelectNextLocation[playerid][td], 0.0000, 51.0000);
		PlayerTextDrawAlignment(playerid, TD_SelectNextLocation[playerid][td], TEXT_DRAW_ALIGN_CENTER);
		PlayerTextDrawColour(playerid, TD_SelectNextLocation[playerid][td], -2139062017);
		PlayerTextDrawBackgroundColour(playerid, TD_SelectNextLocation[playerid][td], -394258433);
		PlayerTextDrawFont(playerid, TD_SelectNextLocation[playerid][td], TEXT_DRAW_FONT_BANK_GOTHIC);
		PlayerTextDrawSetProportional(playerid, TD_SelectNextLocation[playerid][td], true);
		PlayerTextDrawSetShadow(playerid, TD_SelectNextLocation[playerid][td], 0);
...
static TDM_CreateTDSelectSpawnPlayer(playerid)
{
	...
	n_for(i, 5) {
		if(i == 4) {
			// Белый фон группа
			TD_SelectPlayer[playerid][td] = CreatePlayerTextDraw(playerid, initialcoords[0], initialcoords[1], "_");
			PlayerTextDrawFont(playerid, TD_SelectPlayer[playerid][td], TEXT_DRAW_FONT_PREVIEW);
			PlayerTextDrawAlignment(playerid, TD_SelectPlayer[playerid][td], TEXT_DRAW_ALIGN_LEFT);
			PlayerTextDrawColour(playerid, TD_SelectPlayer[playerid][td], 0xFFFFFFFF);
			PlayerTextDrawBackgroundColour(playerid, TD_SelectPlayer[playerid][td], -1717986817);
			PlayerTextDrawSetProportional(playerid, TD_SelectPlayer[playerid][td], false);
			...
			TD_SelectPlayer[playerid][td] = CreatePlayerTextDraw(playerid, initialcoords[0] + 1.0, initialcoords[1] + 1.0, "_");
			PlayerTextDrawFont(playerid, TD_SelectPlayer[playerid][td], TEXT_DRAW_FONT_PREVIEW);
			PlayerTextDrawAlignment(playerid, TD_SelectPlayer[playerid][td], TEXT_DRAW_ALIGN_LEFT);
			PlayerTextDrawColour(playerid, TD_SelectPlayer[playerid][td], 0xFFFFFFFF);
			PlayerTextDrawBackgroundColour(playerid, TD_SelectPlayer[playerid][td], 303174399);
			PlayerTextDrawSetProportional(playerid, TD_SelectPlayer[playerid][td], false);
			PlayerTextDrawSetShadow(playerid, TD_SelectPlayer[playerid][td], 0);
			...
			PlayerTextDrawAlignment(playerid, TD_SelectPlayer[playerid][td], TEXT_DRAW_ALIGN_CENTER);
			PlayerTextDrawColour(playerid, TD_SelectPlayer[playerid][td], -606348801);
			PlayerTextDrawBackgroundColour(playerid, TD_SelectPlayer[playerid][td], 255);
			PlayerTextDrawFont(playerid, TD_SelectPlayer[playerid][td], TEXT_DRAW_FONT_BANK_GOTHIC);
			PlayerTextDrawSetProportional(playerid, TD_SelectPlayer[playerid][td], true);
			PlayerTextDrawSetShadow(playerid, TD_SelectPlayer[playerid][td], 0);
			break;
		}
		...
		PlayerTextDrawAlignment(playerid, TD_SelectPlayer[playerid][td], TEXT_DRAW_ALIGN_CENTER);
		PlayerTextDrawColour(playerid, TD_SelectPlayer[playerid][td], -842150913);
		PlayerTextDrawBackgroundColour(playerid, TD_SelectPlayer[playerid][td], 255);
		PlayerTextDrawFont(playerid, TD_SelectPlayer[playerid][td], TEXT_DRAW_FONT_AHARONI_BOLD);
		PlayerTextDrawSetProportional(playerid, TD_SelectPlayer[playerid][td], true);
		PlayerTextDrawSetShadow(playerid, TD_SelectPlayer[playerid][td], 0);
		td++;

		// Полоска
		TD_SelectPlayer[playerid][td] = CreatePlayerTextDraw(playerid, initialcoords[0] + 1.0, initialcoords[1] + 25.0, "_");
		PlayerTextDrawFont(playerid, TD_SelectPlayer[playerid][td], TEXT_DRAW_FONT_PREVIEW);
		PlayerTextDrawAlignment(playerid, TD_SelectPlayer[playerid][td], TEXT_DRAW_ALIGN_LEFT);
		PlayerTextDrawColour(playerid, TD_SelectPlayer[playerid][td], 0xFFFFFFFF);
		PlayerTextDrawBackgroundColour(playerid, TD_SelectPlayer[playerid][td], -1717986817);
	...
	if(oneidtd != -1) {
		switch(oneidtd) {
			case 0: {
				...
				TD_SelectTeam[playerid][0] = CreatePlayerTextDraw(playerid, 289.0000, 129.0000, "_");
				PlayerTextDrawFont(playerid, TD_SelectTeam[playerid][0], TEXT_DRAW_FONT_PREVIEW);
				PlayerTextDrawAlignment(playerid, TD_SelectTeam[playerid][0], TEXT_DRAW_ALIGN_LEFT);
				PlayerTextDrawColour(playerid, TD_SelectTeam[playerid][0], 0xFFFFFFFF);
				PlayerTextDrawBackgroundColour(playerid, TD_SelectTeam[playerid][0], -1717986817);
				PlayerTextDrawSetProportional(playerid, TD_SelectTeam[playerid][0], false);
				...
				TD_SelectTeam[playerid][1] = CreatePlayerTextDraw(playerid, 290.0000, 130.0000, "_");
				PlayerTextDrawFont(playerid, TD_SelectTeam[playerid][1], TEXT_DRAW_FONT_PREVIEW);
				PlayerTextDrawAlignment(playerid, TD_SelectTeam[playerid][1], TEXT_DRAW_ALIGN_LEFT);
				PlayerTextDrawColour(playerid, TD_SelectTeam[playerid][1], 0xFFFFFFFF);
				PlayerTextDrawBackgroundColour(playerid, TD_SelectTeam[playerid][1], 303174399);
				PlayerTextDrawSetProportional(playerid, TD_SelectTeam[playerid][1], false);
				...
				PlayerTextDrawAlignment(playerid, TD_SelectTeam[playerid][2], TEXT_DRAW_ALIGN_CENTER);
				PlayerTextDrawColour(playerid, TD_SelectTeam[playerid][2], -330419713);
				PlayerTextDrawBackgroundColour(playerid, TD_SelectTeam[playerid][2], 255);
				PlayerTextDrawFont(playerid, TD_SelectTeam[playerid][2], TEXT_DRAW_FONT_BANK_GOTHIC);
				PlayerTextDrawSetProportional(playerid, TD_SelectTeam[playerid][2], true);
				...
				TD_SelectTeam[playerid][3] = CreatePlayerTextDraw(playerid, 318.0000, 150.0000, "_");
				PlayerTextDrawFont(playerid, TD_SelectTeam[playerid][3], TEXT_DRAW_FONT_PREVIEW);
				PlayerTextDrawAlignment(playerid, TD_SelectTeam[playerid][3], TEXT_DRAW_ALIGN_LEFT);
				PlayerTextDrawColour(playerid, TD_SelectTeam[playerid][3], 0xFFFFFFFF);
				PlayerTextDrawBackgroundColour(playerid, TD_SelectTeam[playerid][3], TDM_ShowTeamColorXF(idteam));
				PlayerTextDrawSetProportional(playerid, TD_SelectTeam[playerid][3], false);
				...
				// Задний чёрный фон
				TD_SelectTeam[playerid][4] = CreatePlayerTextDraw(playerid, 319.0000, 151.0000, "_");
				PlayerTextDrawFont(playerid, TD_SelectTeam[playerid][4], TEXT_DRAW_FONT_PREVIEW);
				PlayerTextDrawAlignment(playerid, TD_SelectTeam[playerid][4], TEXT_DRAW_ALIGN_LEFT);
				PlayerTextDrawColour(playerid, TD_SelectTeam[playerid][4], 0xFFFFFFFF);
				PlayerTextDrawBackgroundColour(playerid, TD_SelectTeam[playerid][4], 303174399);
				PlayerTextDrawSetProportional(playerid, TD_SelectTeam[playerid][4], false);
				...
				PlayerTextDrawAlignment(playerid, TD_SelectTeam[playerid][5], TEXT_DRAW_ALIGN_CENTER);
				PlayerTextDrawColour(playerid, TD_SelectTeam[playerid][5], TDM_ShowTeamColorXF(idteam));
				PlayerTextDrawBackgroundColour(playerid, TD_SelectTeam[playerid][5], 255);
				PlayerTextDrawFont(playerid, TD_SelectTeam[playerid][5], TEXT_DRAW_FONT_BANK_GOTHIC);
				PlayerTextDrawSetProportional(playerid, TD_SelectTeam[playerid][5], true);
				...
				PlayerTextDrawAlignment(playerid, TD_SelectTeam[playerid][6], TEXT_DRAW_ALIGN_CENTER);
				PlayerTextDrawColour(playerid, TD_SelectTeam[playerid][6], -505290753);
				PlayerTextDrawBackgroundColour(playerid, TD_SelectTeam[playerid][6], 255);
				PlayerTextDrawFont(playerid, TD_SelectTeam[playerid][6], TEXT_DRAW_FONT_BANK_GOTHIC);
				PlayerTextDrawSetProportional(playerid, TD_SelectTeam[playerid][6], true);
			...
			case 2: {
				// Задний фон цвета команд
				TD_SelectTeam[playerid][11] = CreatePlayerTextDraw(playerid, 318.0000, 245.0000, "_");
				PlayerTextDrawFont(playerid, TD_SelectTeam[playerid][11], TEXT_DRAW_FONT_PREVIEW);
				PlayerTextDrawAlignment(playerid, TD_SelectTeam[playerid][11], TEXT_DRAW_ALIGN_LEFT);
				PlayerTextDrawColour(playerid, TD_SelectTeam[playerid][11], 0xFFFFFFFF);
				PlayerTextDrawBackgroundColour(playerid, TD_SelectTeam[playerid][11], TDM_ShowTeamColorXF(idteam));
				PlayerTextDrawSetProportional(playerid, TD_SelectTeam[playerid][11], false);
				...
				// Задний чёрный фон
				TD_SelectTeam[playerid][12] = CreatePlayerTextDraw(playerid, 319.0000, 246.0000, "_");
				PlayerTextDrawFont(playerid, TD_SelectTeam[playerid][12], TEXT_DRAW_FONT_PREVIEW);
				PlayerTextDrawAlignment(playerid, TD_SelectTeam[playerid][12], TEXT_DRAW_ALIGN_LEFT);
				PlayerTextDrawColour(playerid, TD_SelectTeam[playerid][12], 0xFFFFFFFF);
				PlayerTextDrawBackgroundColour(playerid, TD_SelectTeam[playerid][12], 303174399);
				PlayerTextDrawSetProportional(playerid, TD_SelectTeam[playerid][12], false);
				...
				PlayerTextDrawAlignment(playerid, TD_SelectTeam[playerid][13], TEXT_DRAW_ALIGN_CENTER);
				PlayerTextDrawColour(playerid, TD_SelectTeam[playerid][13], TDM_ShowTeamColorXF(idteam));
				PlayerTextDrawBackgroundColour(playerid, TD_SelectTeam[playerid][13], 255);
				PlayerTextDrawFont(playerid, TD_SelectTeam[playerid][13], TEXT_DRAW_FONT_BANK_GOTHIC);
				PlayerTextDrawSetProportional(playerid, TD_SelectTeam[playerid][13], true);
				...
				PlayerTextDrawAlignment(playerid, TD_SelectTeam[playerid][14], TEXT_DRAW_ALIGN_CENTER);
				PlayerTextDrawColour(playerid, TD_SelectTeam[playerid][14], -505290753);
				PlayerTextDrawBackgroundColour(playerid, TD_SelectTeam[playerid][14], 255);
				PlayerTextDrawFont(playerid, TD_SelectTeam[playerid][14], TEXT_DRAW_FONT_BANK_GOTHIC);
				PlayerTextDrawSetProportional(playerid, TD_SelectTeam[playerid][14], true);
	...
	if(idtd != -1) {
		switch(idtd) {
			case 0: {
				...
				// Задний фон цвета команд
				TD_SelectTeam[playerid][3] = CreatePlayerTextDraw(playerid, 223.0000, 150.0000, "_");
				PlayerTextDrawFont(playerid, TD_SelectTeam[playerid][3], TEXT_DRAW_FONT_PREVIEW);
				PlayerTextDrawAlignment(playerid, TD_SelectTeam[playerid][3], TEXT_DRAW_ALIGN_LEFT);
				PlayerTextDrawColour(playerid, TD_SelectTeam[playerid][3], 0xFFFFFFFF);
				PlayerTextDrawBackgroundColour(playerid, TD_SelectTeam[playerid][3], TDM_ShowTeamColorXF(idteam));
				...
				// Задний чёрный фон
				TD_SelectTeam[playerid][4] = CreatePlayerTextDraw(playerid, 224.0000, 151.0000, "_");
				PlayerTextDrawFont(playerid, TD_SelectTeam[playerid][4], TEXT_DRAW_FONT_PREVIEW);
				PlayerTextDrawAlignment(playerid, TD_SelectTeam[playerid][4], TEXT_DRAW_ALIGN_LEFT);
				PlayerTextDrawColour(playerid, TD_SelectTeam[playerid][4], 0xFFFFFFFF);
				PlayerTextDrawBackgroundColour(playerid, TD_SelectTeam[playerid][4], 303174399);
			...
			case 1: {
				// Задний фон цвета команд
				TD_SelectTeam[playerid][7] = CreatePlayerTextDraw(playerid, 318.0000, 150.0000, "_");
				PlayerTextDrawFont(playerid, TD_SelectTeam[playerid][7], TEXT_DRAW_FONT_PREVIEW);
				PlayerTextDrawAlignment(playerid, TD_SelectTeam[playerid][7], TEXT_DRAW_ALIGN_LEFT);
				PlayerTextDrawColour(playerid, TD_SelectTeam[playerid][7], 0xFFFFFFFF);
				PlayerTextDrawBackgroundColour(playerid, TD_SelectTeam[playerid][7], TDM_ShowTeamColorXF(idteam));
				PlayerTextDrawSetProportional(playerid, TD_SelectTeam[playerid][7], false);
				...
				// Задний чёрный фон
				TD_SelectTeam[playerid][8] = CreatePlayerTextDraw(playerid, 319.0000, 151.0000, "_");
				PlayerTextDrawFont(playerid, TD_SelectTeam[playerid][8], TEXT_DRAW_FONT_PREVIEW);
				PlayerTextDrawAlignment(playerid, TD_SelectTeam[playerid][8], TEXT_DRAW_ALIGN_LEFT);
				PlayerTextDrawColour(playerid, TD_SelectTeam[playerid][8], 0xFFFFFFFF);
				PlayerTextDrawBackgroundColour(playerid, TD_SelectTeam[playerid][8], 303174399);
				PlayerTextDrawSetProportional(playerid, TD_SelectTeam[playerid][8], false);
				...
				PlayerTextDrawAlignment(playerid, TD_SelectTeam[playerid][9], TEXT_DRAW_ALIGN_CENTER);
				PlayerTextDrawColour(playerid, TD_SelectTeam[playerid][9], TDM_ShowTeamColorXF(idteam));
				PlayerTextDrawBackgroundColour(playerid, TD_SelectTeam[playerid][9], 255);
				PlayerTextDrawFont(playerid, TD_SelectTeam[playerid][9], TEXT_DRAW_FONT_BANK_GOTHIC);
				PlayerTextDrawSetProportional(playerid, TD_SelectTeam[playerid][9], true);
				...
				PlayerTextDrawAlignment(playerid, TD_SelectTeam[playerid][10], TEXT_DRAW_ALIGN_CENTER);
				PlayerTextDrawColour(playerid, TD_SelectTeam[playerid][10], -505290753);
				PlayerTextDrawBackgroundColour(playerid, TD_SelectTeam[playerid][10], 255);
				PlayerTextDrawFont(playerid, TD_SelectTeam[playerid][10], TEXT_DRAW_FONT_BANK_GOTHIC);
				PlayerTextDrawSetProportional(playerid, TD_SelectTeam[playerid][10], true);
				PlayerTextDrawSetShadow(playerid, TD_SelectTeam[playerid][10], 0);
				PlayerTextDrawShow(playerid, TD_SelectTeam[playerid][10]);
			}
			case 2: {
				// Задний фон цвета команд
				TD_SelectTeam[playerid][11] = CreatePlayerTextDraw(playerid, 223.0000, 245.0000, "_");
				PlayerTextDrawFont(playerid, TD_SelectTeam[playerid][11], TEXT_DRAW_FONT_PREVIEW);
				PlayerTextDrawAlignment(playerid, TD_SelectTeam[playerid][11], TEXT_DRAW_ALIGN_LEFT);
				PlayerTextDrawColour(playerid, TD_SelectTeam[playerid][11], 0xFFFFFFFF);
				PlayerTextDrawBackgroundColour(playerid, TD_SelectTeam[playerid][11], TDM_ShowTeamColorXF(idteam));
				...
				// Задний чёрный фон
				TD_SelectTeam[playerid][12] = CreatePlayerTextDraw(playerid, 224.0000, 246.0000, "_");
				PlayerTextDrawFont(playerid, TD_SelectTeam[playerid][12], TEXT_DRAW_FONT_PREVIEW);
				PlayerTextDrawAlignment(playerid, TD_SelectTeam[playerid][12], TEXT_DRAW_ALIGN_LEFT);
				PlayerTextDrawColour(playerid, TD_SelectTeam[playerid][12], 0xFFFFFFFF);
				PlayerTextDrawBackgroundColour(playerid, TD_SelectTeam[playerid][12], 303174399);
			...
			case 3: {
				// Задний фон цвета команд
				TD_SelectTeam[playerid][15] = CreatePlayerTextDraw(playerid, 318.0000, 245.0000, "_");
				PlayerTextDrawFont(playerid, TD_SelectTeam[playerid][15], TEXT_DRAW_FONT_PREVIEW);
				PlayerTextDrawAlignment(playerid, TD_SelectTeam[playerid][15], TEXT_DRAW_ALIGN_LEFT);
				PlayerTextDrawColour(playerid, TD_SelectTeam[playerid][15], 0xFFFFFFFF);
				PlayerTextDrawBackgroundColour(playerid, TD_SelectTeam[playerid][15], TDM_ShowTeamColorXF(idteam));
				PlayerTextDrawSetProportional(playerid, TD_SelectTeam[playerid][15], false);
				...
				// Задний чёрный фон
				TD_SelectTeam[playerid][16] = CreatePlayerTextDraw(playerid, 319.0000, 246.0000, "_");
				PlayerTextDrawFont(playerid, TD_SelectTeam[playerid][16], TEXT_DRAW_FONT_PREVIEW);
				PlayerTextDrawAlignment(playerid, TD_SelectTeam[playerid][16], TEXT_DRAW_ALIGN_LEFT);
				PlayerTextDrawColour(playerid, TD_SelectTeam[playerid][16], 0xFFFFFFFF);
				PlayerTextDrawBackgroundColour(playerid, TD_SelectTeam[playerid][16], 303174399);
				PlayerTextDrawSetProportional(playerid, TD_SelectTeam[playerid][16], false);
				...
				PlayerTextDrawAlignment(playerid, TD_SelectTeam[playerid][17], TEXT_DRAW_ALIGN_CENTER);
				PlayerTextDrawColour(playerid, TD_SelectTeam[playerid][17], TDM_ShowTeamColorXF(idteam));
				PlayerTextDrawBackgroundColour(playerid, TD_SelectTeam[playerid][17], 255);
				PlayerTextDrawFont(playerid, TD_SelectTeam[playerid][17], TEXT_DRAW_FONT_BANK_GOTHIC);
				PlayerTextDrawSetProportional(playerid, TD_SelectTeam[playerid][17], true);
				...
				PlayerTextDrawAlignment(playerid, TD_SelectTeam[playerid][18], TEXT_DRAW_ALIGN_CENTER);
				PlayerTextDrawColour(playerid, TD_SelectTeam[playerid][18], -505290753);
				PlayerTextDrawBackgroundColour(playerid, TD_SelectTeam[playerid][18], 255);
				PlayerTextDrawFont(playerid, TD_SelectTeam[playerid][18], TEXT_DRAW_FONT_BANK_GOTHIC);
				PlayerTextDrawSetProportional(playerid, TD_SelectTeam[playerid][18], true);
...
static CreatePlayerTDResultChet(playerid, team_id, bool:td_id, num, teams)
{
	...
	// Задний фон
	TD_ResultRound[playerid][num][tdid] = CreatePlayerTextDraw(playerid, pos_td[0], pos_td[1], "_");
	PlayerTextDrawFont(playerid, TD_ResultRound[playerid][num][tdid], TEXT_DRAW_FONT_PREVIEW);
	PlayerTextDrawAlignment(playerid, TD_ResultRound[playerid][num][tdid], TEXT_DRAW_ALIGN_LEFT);
	PlayerTextDrawColour(playerid, TD_ResultRound[playerid][num][tdid], 0xFFFFFFFF);
	PlayerTextDrawBackgroundColour(playerid, TD_ResultRound[playerid][num][tdid], 0x303030FF);
	PlayerTextDrawSetProportional(playerid, TD_ResultRound[playerid][num][tdid], false);
	...
	TD_ResultRound[playerid][num][tdid] = CreatePlayerTextDraw(playerid, pos_td[0], pos_td[1] + 16.0, "_");
	PlayerTextDrawFont(playerid, TD_ResultRound[playerid][num][tdid], TEXT_DRAW_FONT_PREVIEW);
	PlayerTextDrawAlignment(playerid, TD_ResultRound[playerid][num][tdid], TEXT_DRAW_ALIGN_LEFT);
	PlayerTextDrawColour(playerid, TD_ResultRound[playerid][num][tdid], 0xFFFFFFFF);
	PlayerTextDrawBackgroundColour(playerid, TD_ResultRound[playerid][num][tdid], TDM_ShowTeamColorXB(team_id));
	PlayerTextDrawSetProportional(playerid, TD_ResultRound[playerid][num][tdid], false);
	PlayerTextDrawSetShadow(playerid, TD_ResultRound[playerid][num][tdid], 0);
	...
	TD_ResultRound[playerid][num][tdid] = CreatePlayerTextDraw(playerid, pos_td[0], pos_td[1] + 27.0, "_");
	PlayerTextDrawFont(playerid, TD_ResultRound[playerid][num][tdid], TEXT_DRAW_FONT_PREVIEW); 
	PlayerTextDrawAlignment(playerid, TD_ResultRound[playerid][num][tdid], TEXT_DRAW_ALIGN_LEFT);
	PlayerTextDrawColour(playerid, TD_ResultRound[playerid][num][tdid], 0xFFFFFFFF);
	PlayerTextDrawBackgroundColour(playerid, TD_ResultRound[playerid][num][tdid], TDM_ShowTeamColorXB(team_id));
	...
	// Ники и убийства
	TD_ResultRound[playerid][num][tdid] = CreatePlayerTextDraw(playerid, pos_td[0] + 100.0, pos_td[1] + 17.0, "_");
	PlayerTextDrawFont(playerid, TD_ResultRound[playerid][num][tdid], TEXT_DRAW_FONT_PREVIEW);
	PlayerTextDrawAlignment(playerid, TD_ResultRound[playerid][num][tdid], TEXT_DRAW_ALIGN_LEFT);
	PlayerTextDrawColour(playerid, TD_ResultRound[playerid][num][tdid], 0xFFFFFFFF);
	PlayerTextDrawBackgroundColour(playerid, TD_ResultRound[playerid][num][tdid], TDM_ShowTeamColorXB(team_id));
	...
	// Убийства и смерти
	TD_ResultRound[playerid][num][tdid] = CreatePlayerTextDraw(playerid, pos_td[0] + 146.0, pos_td[1] + 17.0, "_");
	PlayerTextDrawFont(playerid, TD_ResultRound[playerid][num][tdid], TEXT_DRAW_FONT_PREVIEW);
	PlayerTextDrawAlignment(playerid, TD_ResultRound[playerid][num][tdid], TEXT_DRAW_ALIGN_LEFT);
	PlayerTextDrawColour(playerid, TD_ResultRound[playerid][num][tdid], 0xFFFFFFFF);
	PlayerTextDrawBackgroundColour(playerid, TD_ResultRound[playerid][num][tdid], TDM_ShowTeamColorXB(team_id));
	...
	PlayerTextDrawAlignment(playerid, TD_ResultRound[playerid][num][tdid], TEXT_DRAW_ALIGN_CENTER);
	PlayerTextDrawColour(playerid, TD_ResultRound[playerid][num][tdid], -909522945);
	PlayerTextDrawBackgroundColour(playerid, TD_ResultRound[playerid][num][tdid], 255);
	PlayerTextDrawFont(playerid, TD_ResultRound[playerid][num][tdid], TEXT_DRAW_FONT_BANK_GOTHIC);
	PlayerTextDrawSetProportional(playerid, TD_ResultRound[playerid][num][tdid], true);
	PlayerTextDrawSetShadow(playerid, TD_ResultRound[playerid][num][tdid], 0);
	tdid++;
	...
	PlayerTextDrawAlignment(playerid, TD_ResultRound[playerid][num][tdid], TEXT_DRAW_ALIGN_CENTER);
	PlayerTextDrawColour(playerid, TD_ResultRound[playerid][num][tdid], TDM_ShowTeamColorXF(team_id));
	PlayerTextDrawBackgroundColour(playerid, TD_ResultRound[playerid][num][tdid], 255);
	PlayerTextDrawFont(playerid, TD_ResultRound[playerid][num][tdid], TEXT_DRAW_FONT_BANK_GOTHIC);
	PlayerTextDrawSetProportional(playerid, TD_ResultRound[playerid][num][tdid], true);
	PlayerTextDrawSetShadow(playerid, TD_ResultRound[playerid][num][tdid], 0);
	...
	if(teams == (num + 1)) {
		...
		TD_EndRoundStats[playerid][0] = CreatePlayerTextDraw(playerid, pos_td_stats[0], pos_td_stats[1] + 144.0, "_");
		PlayerTextDrawFont(playerid, TD_EndRoundStats[playerid][0], TEXT_DRAW_FONT_PREVIEW);
		PlayerTextDrawAlignment(playerid, TD_EndRoundStats[playerid][0], TEXT_DRAW_ALIGN_LEFT);
		PlayerTextDrawColour(playerid, TD_EndRoundStats[playerid][0], 0xFFFFFFFF);
		PlayerTextDrawBackgroundColour(playerid, TD_EndRoundStats[playerid][0], 0x303030FF);
		PlayerTextDrawSetProportional(playerid, TD_EndRoundStats[playerid][0], false);
		...
		PlayerTextDrawAlignment(playerid, TD_EndRoundStats[playerid][1], TEXT_DRAW_ALIGN_LEFT);
		PlayerTextDrawColour(playerid, TD_EndRoundStats[playerid][1], 0xFFFFFFFF);
		PlayerTextDrawBackgroundColour(playerid, TD_EndRoundStats[playerid][1], 255);
		PlayerTextDrawFont(playerid, TD_EndRoundStats[playerid][1], TEXT_DRAW_FONT_BANK_GOTHIC);
		PlayerTextDrawSetProportional(playerid, TD_EndRoundStats[playerid][1], true);
		...
		PlayerTextDrawAlignment(playerid, TD_EndRoundStats[playerid][2], TEXT_DRAW_ALIGN_LEFT);
		PlayerTextDrawColour(playerid, TD_EndRoundStats[playerid][2], 0xFFFFFFFF);
		PlayerTextDrawBackgroundColour(playerid, TD_EndRoundStats[playerid][2], 255);
		PlayerTextDrawFont(playerid, TD_EndRoundStats[playerid][2], TEXT_DRAW_FONT_BANK_GOTHIC);
		PlayerTextDrawSetProportional(playerid, TD_EndRoundStats[playerid][2], true);
		...
		PlayerTextDrawAlignment(playerid, TD_EndRoundStats[playerid][3], TEXT_DRAW_ALIGN_LEFT);
		PlayerTextDrawColour(playerid, TD_EndRoundStats[playerid][3], 0xFFFFFFFF);
		PlayerTextDrawBackgroundColour(playerid, TD_EndRoundStats[playerid][3], 255);
		PlayerTextDrawFont(playerid, TD_EndRoundStats[playerid][3], TEXT_DRAW_FONT_BANK_GOTHIC);
		PlayerTextDrawSetProportional(playerid, TD_EndRoundStats[playerid][3], true);
		PlayerTextDrawSetShadow(playerid, TD_EndRoundStats[playerid][3], 0);

		// Полоска
		TD_EndRoundStats[playerid][4] = CreatePlayerTextDraw(playerid, pos_td_stats[0] + 66.0, pos_td_stats[1] + 144.0, "_");
		PlayerTextDrawFont(playerid, TD_EndRoundStats[playerid][4], TEXT_DRAW_FONT_PREVIEW);
		PlayerTextDrawAlignment(playerid, TD_EndRoundStats[playerid][4], TEXT_DRAW_ALIGN_LEFT);
		PlayerTextDrawColour(playerid, TD_EndRoundStats[playerid][4], 0xFFFFFFFF);
		PlayerTextDrawBackgroundColour(playerid, TD_EndRoundStats[playerid][4], 2021161215);
		PlayerTextDrawSetProportional(playerid, TD_EndRoundStats[playerid][4], false);
		...
		PlayerTextDrawAlignment(playerid, TD_EndRoundStats[playerid][5], TEXT_DRAW_ALIGN_LEFT);
		PlayerTextDrawColour(playerid, TD_EndRoundStats[playerid][5], 0xFFFFFFFF);
		PlayerTextDrawBackgroundColour(playerid, TD_EndRoundStats[playerid][5], 255);
		PlayerTextDrawFont(playerid, TD_EndRoundStats[playerid][5], TEXT_DRAW_FONT_BANK_GOTHIC);
		PlayerTextDrawSetProportional(playerid, TD_EndRoundStats[playerid][5], true);
		...
		PlayerTextDrawAlignment(playerid, TD_EndRoundStats[playerid][6], TEXT_DRAW_ALIGN_LEFT);
		PlayerTextDrawColour(playerid, TD_EndRoundStats[playerid][6], 0xFFFFFFFF);
		PlayerTextDrawBackgroundColour(playerid, TD_EndRoundStats[playerid][6], 255);
		PlayerTextDrawFont(playerid, TD_EndRoundStats[playerid][6], TEXT_DRAW_FONT_BANK_GOTHIC);
		PlayerTextDrawSetProportional(playerid, TD_EndRoundStats[playerid][6], true);
	...
	n_for(i, 10) {
		...
		PlayerTextDrawAlignment(playerid, TD_TopKillers[playerid][num][tdid][0], TEXT_DRAW_ALIGN_LEFT);
		PlayerTextDrawColour(playerid, TD_TopKillers[playerid][num][tdid][0], -909522945);
		PlayerTextDrawBackgroundColour(playerid, TD_TopKillers[playerid][num][tdid][0], 255);
		PlayerTextDrawFont(playerid, TD_TopKillers[playerid][num][tdid][0], TEXT_DRAW_FONT_BANK_GOTHIC);
		PlayerTextDrawSetProportional(playerid, TD_TopKillers[playerid][num][tdid][0], true);
		...
		PlayerTextDrawAlignment(playerid, TD_TopKillers[playerid][num][tdid][1], TEXT_DRAW_ALIGN_CENTER);
		PlayerTextDrawColour(playerid, TD_TopKillers[playerid][num][tdid][1], -909522945);
		PlayerTextDrawBackgroundColour(playerid, TD_TopKillers[playerid][num][tdid][1], 255);
		PlayerTextDrawFont(playerid, TD_TopKillers[playerid][num][tdid][1], TEXT_DRAW_FONT_BANK_GOTHIC);
		PlayerTextDrawSetProportional(playerid, TD_TopKillers[playerid][num][tdid][1], true);
		...
		PlayerTextDrawAlignment(playerid, TD_TopKillers[playerid][num][tdid][2], TEXT_DRAW_ALIGN_CENTER);
		PlayerTextDrawColour(playerid, TD_TopKillers[playerid][num][tdid][2], -909522945);
		PlayerTextDrawBackgroundColour(playerid, TD_TopKillers[playerid][num][tdid][2], 255);
		PlayerTextDrawFont(playerid, TD_TopKillers[playerid][num][tdid][2], TEXT_DRAW_FONT_BANK_GOTHIC);
		PlayerTextDrawSetProportional(playerid, TD_TopKillers[playerid][num][tdid][2], true);
			...
			if(player_squad > -1) {
				...
				&& !ActionPlayerSelectTP{player_squad}) {
					if(ActiveCellSelectPlayerID[playerid] != player_squad) {
						...
						PlayerTextDrawBackgroundColour(playerid, TD_SelectPlayer[playerid][ActiveCellSelectPlayerTD[playerid]], 0x3cba40FF);
						PlayerTextDrawShow(playerid, TD_SelectPlayer[playerid][ActiveCellSelectPlayerTD[playerid]]);
