	...
	n_for(w, MAX_PLAYER_WEAPON_SLOTS) {
		...
		if(weapon > 0 && ammo > 0) {
			n_for2(id, DROP_WEAPONS_MAX) {
				...
				dw_PlayerInfo[playerid][id][WEAPON_ObjectWeapon] = CreateDynamicObject(GetWeaponModelEx(weapon), x, y, z - 1.0, 90.000, 0.000, GetPlayerRandomTurn() + GetPlayerRandomTurn2(), dw_PlayerInfo[playerid][id][WEAPON_VirtualWorld], dw_PlayerInfo[playerid][id][WEAPON_Interior]);
...
static ChangePlayerDropWeapon(slotid, id, weaponid, ammo)
{
	...
	DestroyDynamicObject(dw_PlayerInfo[slotid][id][WEAPON_ObjectWeapon]);
	dw_PlayerInfo[slotid][id][WEAPON_ObjectWeapon] = CreateDynamicObject(GetWeaponModelEx(weaponid), PosX, PosY, PosZ, RotX, RotY, RotZ, dw_PlayerInfo[slotid][id][WEAPON_VirtualWorld], dw_PlayerInfo[slotid][id][WEAPON_Interior]);
			...
			m_for(M_GP(playerid), M_GPS(playerid), slotid) {
				foreach(new id:PlayerDropWeapons[slotid]) {
					if(IsPlayerInRangeOfPoint(playerid, 1.0, dw_PlayerInfo[slotid][id][WEAPON_PosX], dw_PlayerInfo[slotid][id][WEAPON_PosY], dw_PlayerInfo[slotid][id][WEAPON_PosZ])) {
						...
						GetPlayerWeaponData(playerid, GetWeaponSlotEx(dw_PlayerInfo[slotid][id][WEAPON_WeaponID]), player_weapon, player_ammo);
