...
Enums:
	-
Globar var:
	pAntiCheatLastCodeTriggerTime
	pAntiCheatSettingsPage
	pAntiCheatSettingsMenuListData
	pAntiCheatSettingsEditCodeId
Commands:
	-
Dialogs:
	AC_Settings
...
#if defined _INC_ANTICHEAT_SYSTEM
	#endinput
#endif
#define _INC_ANTICHEAT_SYSTEM
...
publics OnCheatDetected(playerid, ip_address[], type, code)
{
	if(type == AC_GLOBAL_TRIGGER_TYPE_PLAYER) {
		switch(code) {
			...
			// Sandbox
			case 40: {
				Dialog_Message(playerid, "{CC0033}Античит", MAX_CONNECTS_MSG, "Ok");
				return KickEx(playerid);
			}
			// Invalid Version
			case 41: {
				Dialog_Message(playerid, "{CC0033}Античит", UNKNOWN_CLIENT_MSG, "Ok");
				return KickEx(playerid);
			}
			default: {
				...
				if(trigger_type == AC_CODE_TRIGGER_TYPE_WARNING) {
					f(string, "[Античит]:{FFFFFF} %s[%d] подозревается в использовании чит-программ: %s [Код: %03d]", NameEx(playerid), playerid, AC_CODE_NAME[code], code);
					Adm_SendMessages(0xcc1836FF, string);
					Adm_AddCheater(playerid);
				}
				if(trigger_type == AC_CODE_TRIGGER_TYPE_KICK) {
					f(string, "[Античит]:{FFFFFF} %s[%d] был кикнут по подозрению в использовании чит-программ: %s [Код: %03d].", NameEx(playerid), playerid, AC_CODE_NAME[code], code);
					Adm_SendMessages(0xcc1836FF, string);

					new
						ac_strtmp[300];

					f(ac_strtmp, KICK_MSG, code, code);
					Dialog_Message(playerid, "{CC0033}Античит", ac_strtmp, "Ok");
	...
	// AC_GLOBAL_TRIGGER_TYPE_IP
		...
		f(string, "[Античит]:{FFFFFF} IP-адрес %s был заблокирован: %s [Код: %03d].", ip_address, AC_CODE_NAME[code], code);
		Adm_SendMessages(0xCC0033FF, string);

		new
			ac_strtmp[300];

		f(ac_strtmp, KICK_MSG, code, code);
		Dialog_Message(playerid, "{CC0033}Античит / Anti-cheat", ac_strtmp, "Ok");
...
DialogCreate:AC_Settings(playerid)
{
	static
	...
	Dialog_Open(playerid, Dialog:AC_Settings, DSTH, "Настройки античита", dialog_string, "Выбрать", "Отмена");
	return 1;
}

DialogResponse:AC_Settings(playerid, response, listitem, inputtext[])
{
	if(!response) {
	...
	if(!strcmp(inputtext, AC_DIALOG_NEXT_PAGE_TEXT))
		...
		return Dialog_Show(playerid, Dialog:AC_CheatEditCode);
	}
	return Dialog_Show(playerid, Dialog:AC_Settings);
}

DialogCreate:AC_CheatEditCode(playerid)
{
	new
	...
	Dialog_Open(playerid, Dialog:AC_CheatEditCode, DSL, dialog_header, dialog_string, "Выбрать", "Назад");
	return 1;
}

DialogResponse:AC_CheatEditCode(playerid, response, listitem, inputtext[])
{
	if(!response) {
