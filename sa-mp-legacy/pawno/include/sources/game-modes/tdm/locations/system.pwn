...
Functions:
	...
	Stock:
		...
		TDM_SetSpawnTopBot(session_id, cell, Float:X, Float:Y, Float:Z, Float:Angle)
		TDM_GetSpawnTopBot(session_id, cell, &Float:X, &Float:Y, &Float:Z, &Float:Angle)
...
static
	Float:BotsTopKillsPos[TDM_MAX_GAME_SESSIONS][5][4],
...
static CreatePlayerGangZone(playerid, gangzone_id)
{
	if(GangZonePlayerCapture[playerid] != -1)
		DestroyPlayerGangZone(playerid);
...
static DestroyPlayerGangZone(playerid)
{
	if(GangZonePlayerCapture[playerid] == -1)
		return 0;
...
static CreatePlayerComputer(playerid, team_id, comp_id) 
{
	...
	PlayerTextDrawColor(playerid, TD_ComputerHack[playerid], TDM_ShowTeamColorXB(team_id));
...
stock TDM_SetSpawnTopBot(session_id, cell, Float:X, Float:Y, Float:Z, Float:Angle)
{
	BotsTopKillsPos[session_id][cell][0] = X;
	BotsTopKillsPos[session_id][cell][1] = Y;
	BotsTopKillsPos[session_id][cell][2] = Z;
	BotsTopKillsPos[session_id][cell][3] = Angle;
	return 1;
}

stock TDM_GetSpawnTopBot(session_id, cell, &Float:X, &Float:Y, &Float:Z, &Float:Angle)
{
	X = BotsTopKillsPos[session_id][cell][0];
	Y = BotsTopKillsPos[session_id][cell][1];
	Z = BotsTopKillsPos[session_id][cell][2];
	Angle = BotsTopKillsPos[session_id][cell][3];
	return 1;
}

static TDM_ResetSpawnTopBot(session_id)
{
	n_for(i, sizeof(BotsTopKillsPos[])) {
		BotsTopKillsPos[session_id][i][0] =
		BotsTopKillsPos[session_id][i][1] =
		BotsTopKillsPos[session_id][i][2] =
		BotsTopKillsPos[session_id][i][3] = 0.0;
	}
	return 1;
}
...
stock TDM_CreateElementEnterArea(playerid)
{
	...
	if(Mode_GetMode(MODE_TDM, session_id) == TDM_MODE_CAPTURE) {
		...
		if(point_id > -1)
			CreatePlayerGangZone(playerid, point_id);
	}
	if(TDM_GetExitZone(session_id)) {
...
static ShowTDHackComputer(playerid)
{
	...
	PlayerTextDrawAlignment(playerid, TD_ComputerHack[playerid], 2);
	PlayerTextDrawColor(playerid, TD_ComputerHack[playerid], -1);
	PlayerTextDrawSetShadow(playerid, TD_ComputerHack[playerid], 0);
	PlayerTextDrawSetOutline(playerid, TD_ComputerHack[playerid], 0);
	PlayerTextDrawBackgroundColor(playerid, TD_ComputerHack[playerid], 255);
	PlayerTextDrawFont(playerid, TD_ComputerHack[playerid], 2);
	PlayerTextDrawSetProportional(playerid, TD_ComputerHack[playerid], 1);
	return 1;
}
...
static ShowTDGangZoneCapture(playerid)
{
	...
	PlayerTextDrawAlignment(playerid, TD_GangZone[playerid], 2);
	PlayerTextDrawColor(playerid, TD_GangZone[playerid], -1);
	PlayerTextDrawBackgroundColor(playerid, TD_GangZone[playerid], 255);
	PlayerTextDrawFont(playerid, TD_GangZone[playerid], 2);
	PlayerTextDrawSetProportional(playerid, TD_GangZone[playerid], 1);
...
stock TDM_LocOnGameModeInit(session_id)
{
	...
	TDM_ResetTimer(session_id);
	TDM_ResetExitZone(session_id);
	TDM_ResetSpawnTopBot(session_id);
	TDM_ResetCameraEndPos(session_id);
...
stock TDM_LocOnPlEnterDynamicArea(playerid, STREAMER_TAG_AREA:areaid)
{
	...
	if(Mode_GetMode(MODE_TDM, session_id) == TDM_MODE_CAPTURE) {
		...
		&& !Adm_GetPlayerSpectating(playerid)) {
			...
			if(gangzone != -1)
				CreatePlayerGangZone(playerid, gangzone);
		}
	}
